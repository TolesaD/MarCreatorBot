...
// Add Express web server for cPanel compatibility
...
function startWebServer() {
  ...
  webApp.get('/', (req, res) => {
    res.json({
      status: 'online',
      application: 'MetaBot Creator',
      message: 'Telegram Bot Manager is running',
      mainBot: process.env.MAIN_BOT || 'MarCreatorBot',
  ...
  webApp.get('/status', (req, res) => {
    const MiniBotManager = require('./services/MiniBotManager');
    const activeBots = Array.from(MiniBotManager.activeBots.entries()).map(([id, bot]) => ({
      id: id,
      name: bot.botRecord?.bot_name || 'Unknown',
      status: bot.instance ? 'active' : 'inactive',
      username: bot.botRecord?.bot_username || 'Unknown'
    }));
...
// Modify the startApplication function to include web server
async function startApplication() {
  try {
    console.log('🔧 Starting MetaBot Creator application...');
    console.log('🚀 Optimized for Yegara.com cPanel deployment');
    ...
    // Then start the bot application
    const app = new MetaBotCreator();
    await app.initialize();
    app.start();
